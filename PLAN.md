...
1. Add `progress_pct` column to `Review` model + write and run the Alembic migration; verify with `alembic upgrade head` and a manual `psql`/`\d reviews` check.
...
- **Output (backend):** `review.progress_pct` persisted in Postgres and returned by `/status` as a real, increasing integer (0 → 100) instead of a hardcoded 0; existing `status` field behavior is unchanged.
